...
const scoresAcrossRunsByCategory = reports['anonymous'].json.reduce((acc, run) => {
...
ciStdout.push(
  "------------------------------------------",
  `Number of parallel test runs (${'anonymous'}): ${reports['anonymous'].json.length}`,
  "------------------------------------------"
);

prComment.push(
  `<h2>Lighthouse scores (${'anonymous'})</h2>`,
  `<p>Best scores across <strong>${reports['anonymous'].json.length}</strong> parallel runs:</p>`,
  '<p>'
);
...
const reportLinks = reports['anonymous'].htmlFilenames.map((filename, idx) => {
