...
**A:** Great question! IPFS is a best-effort CDN, which is permissionless and free to use. IPFS comes with no guarantees in terms of content availability, due to NATs, as well as network churn, but also in terms of delivery latency, due to user's flaky and often slow connections. Despite these well-known challenges of P2P networks, it is being used by thousands of users daily and transferring terabytes of data per month. Check this [guide](https://docs.ipfs.io/concepts/usage-ideas-examples) for all of the things one can do with IPFS.
...
We highly recommend watching [this list of top quality videos](https://www.youtube.com/playlist?list=PL_0VrY55uV19ylyoeS9C9nDsgxNFTU6X2) from some of the top researchers, implementers, and think tanks in the area talking about Filecoin and the impressive milestones that the Filecoin project has just achieved.
...
Note, however, that each replica comes with its own storage cost and therefore the more the replicas, the higher the availability of the data, but the higher the overall cost of storage too.
